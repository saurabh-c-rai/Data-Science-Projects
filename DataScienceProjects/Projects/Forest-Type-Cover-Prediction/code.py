...
# print(X_train.shape)
# scaler=StandardScaler()
# numerical_column=X_train.select_dtypes(include=np.number)
# categorical_coulmn=X_train.select_dtypes(exclude=np.number)
# print(numerical_column.shape)
# print(categorical_coulmn.shape)
# # X_train_temp=scaler.fit_transform(numerical_column)
# # X_train1=pd.concat([pd.DataFrame(X_train_temp),categorical_coulmn],1)
# # print(X_train1)
# # numerical_column_test=X_test.select_dtypes(include=np.number)
# # categorical_coulmn=X_test.select_dtypes(exclude=np.number)
# # X_test_temp=scaler.fit_transform(numerical_column_test)
# # X_test1=pd.concat([pd.DataFrame(X_test_temp),categorical_coulmn],1)
# # print(X_test1)
# # # Scales are not the same for all variables. Hence, rescaling and standardization may be necessary for some algorithm to be applied on it.
...
top_k_predictors = list(dataframe['Features'].iloc[:predictors.shape[1]])
