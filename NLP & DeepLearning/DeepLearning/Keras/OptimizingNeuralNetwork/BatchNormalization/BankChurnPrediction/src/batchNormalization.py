...
final_model.add(
    Dense(activation="relu", input_dim=11, units=32, kernel_initializer="uniform")
)
...
final_model.add(
    Dense(activation="relu", input_dim=11, units=16, kernel_initializer="uniform")
)
...
final_model.add(Dense(1, activation="sigmoid", kernel_initializer="uniform"))
...
final_model.fit(x=X_train, y=y_train, batch_size=32, epochs=100, verbose=1)

# %%
# evaluating on unknown data
score = final_model.evaluate(x=X_test, y=y_test)
