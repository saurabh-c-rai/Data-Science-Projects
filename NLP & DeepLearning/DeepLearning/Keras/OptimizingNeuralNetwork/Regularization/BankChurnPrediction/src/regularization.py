...
classifier.add(Dense(activation="relu", input_shape=(INPUT_LAYER,), units=6))
classifier.add(Dense(activation="relu", input_dim=11, units=6))
classifier.add(Dense(1, activation="sigmoid"))

#%%
# Creating a Stocastic Gradient Descent optimizer
opt = optimizers.SGD(learning_rate=0.01, momentum=0, nesterov=False)
...
classifier.fit(x=X_train, y=y_train, batch_size=32, epochs=100, verbose=1)

# %%
# evaluating on unknown data
score = classifier.evaluate(x=X_test, y=y_test)
